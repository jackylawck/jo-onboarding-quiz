...
if not st.session_state.authenticated:
    ...
    if st.button("確認"):
        if user_code == st.secrets.get("ACCESS_CODE", "HK2026Pass"):
...
def generate_pdf(name, dept, score, total, pass_rate, is_pass):
    ...
    pdf.cell(200, 10, txt=f"得分：{score} / {total}", ln=True)
...
if submit_button:
    if not name or not dept:
        ...
        pass_rate = (score / total_questions) * 100
        is_pass = pass_rate >= 80
        
        st.success(f"提交成功！得分：{score} / {total_questions}（合格率：{pass_rate:.1f}%）")
        if is_pass:
            st.balloons()
            st.success("🎉 恭喜通過入職培訓考核！")
        else:
            st.error("⚠️ 未達 80% 合格標準。")
            
        try:
            pdf_bytes = generate_pdf(name, dept, score, total_questions, pass_rate, is_pass)
